...
interface CustomFormInputObject {
    type: 'input';
    /** 输入框标题 */
    title: string;
    /** 输入框中显示的提示文本 */
...
interface CustomFormSwitchObject {
    type: 'switch';
    /** 开关标题 */
    title: string;
    /** 开关默认状态，默认为 `false` */
...
interface CustomFormDropdownObject {
    type: 'dropdown';
    /** 下拉框标题 */
    title: string;
    /** 下拉框元素 */
...
interface CustomFormSliderObject {
    type: 'slider';
    /** 滑块标题 */
    title: string;
    /** 滑块最小值 */
...
interface CustomFormStepSliderObject {
    type: 'stepSlider';
    /** 步进滑块标题 */
    title: string;
    /** 步进滑块元素列表 */
...
type CustomFormObject = CustomFormLabelObject | CustomFormInputObject | CustomFormSwitchObject | CustomFormDropdownObject | CustomFormSliderObject | CustomFormStepSliderObject;
/**
 ...
 * @returns 构建好的表单
 */
declare function buildCustomForm(formTitle: string, objects: CustomFormObject[]): CustomForm;
type CustomFormObjectReturnType<T extends CustomFormObject> = T extends CustomFormInputObject ? string : T extends CustomFormSwitchObject ? boolean : T extends CustomFormDropdownObject | CustomFormSliderObject | CustomFormStepSliderObject ? number : undefined;
type CustomFormReturn<T extends {
    [id: string]: CustomFormObject;
}> = {
    [k in keyof T]: CustomFormObjectReturnType<T[k]>;
};
interface CustomFormInputOptions {
    /** 输入框中显示的提示文本 */
    placeholder?: string;
    /** 输入框默认内容 */
    default?: string;
}
interface CustomFormSliderOptions {
    /** 滑块滑动一格加减的数值，默认为 `1` */
    step?: number;
    /** 滑块默认位置，默认为 `min` */
    default?: number;
}
declare class CustomFormEx<T extends {
    [id: string]: CustomFormObject;
} = {}> {
    #private;
    /** 表单标题 */
 ...
 * @returns 格式化后的错误
 */
declare function formatError(e: unknown): string;
/**
 * 在 sync function 中使用 setTimeout 调用 async function，解决 LLSE 回调调用 async 函数会出现的玄学 bug
 * @param func async function
 * @returns wrapped sync function
 */
declare function wrapAsyncFunc<T extends unknown[]>(func: (...args: T) => Promise<unknown>): (...args: T) => void;
/**
