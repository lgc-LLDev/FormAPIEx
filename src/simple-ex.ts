...
export class SimpleFormEx<T> {
  ...
  public formatter: (v: T, index: number, array: T[]) => [string, string?] = (v) => [
    `§3${String(v)}`,
  ]
   ...
   * @returns 搜索到的按钮参数列表
   */
  public searcher: (buttons: T[], param: string) => T[] = (buttons, param): T[] => {
   ...
   */
  async sendSearchForm(player: Player, defaultVal = ''): Promise<T | null | FormClose> {
