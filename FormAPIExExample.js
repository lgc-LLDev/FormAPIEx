...
mc.listen('onServerStarted', () => {
  ...
  cmdTestModalForm.setCallback((_, { player }) => {
    ...
    wrapAsyncFunc(async () => {
      const result = await sendModalFormAsync(player, PLUGIN_NAME, 'A test ModalForm~')
  ...
  const cmdTestCustomForm = mc.newCommand('testcustom', PLUGIN_NAME, PermType.Any)
  ...
  cmdTestCustomForm.setCallback((_, { player }) => {
    ...
    */
      ...
      player.tell(res === FormClose ? String(res) : JSON.stringify(res, null, 2))
    })()
  ...
  // 快速构建一个列表表单，带搜索带翻页
  const cmdTestSimpleFormEx = mc.newCommand('testsimple', PLUGIN_NAME, PermType.Any)
  ...
  // 构建一个功能性 SimpleForm
  const cmdTestSimpleFormEx2 = mc.newCommand('testsimple2', PLUGIN_NAME, PermType.Any)
  cmdTestSimpleFormEx2.overload()
  cmdTestSimpleFormEx2.setCallback((_, { player }) => {
    if (!player) return false

    wrapAsyncFunc(async () => {
      const form = new SimpleFormOperational(PLUGIN_NAME, '§a请选择要执行的操作：', [
